...
domains
    age = integer
    name, gender, job, weapon, reason, smear, item = symbol

predicates
    nondeterm person(name, age, gender, job).
...
clauses
    ...
    /* Некоторые факты */
    operates_identically(wooden_leg,club).        % работает_одинаково(деревянная_нога,дубинка)
    operates_identically(bar,club).               % работает_одинаково(брусок,дубинка)
    operates_identically(pair_of_scissors,knife). % работает_одинаково(ножницы,нож)
    operates_identically(football_boot,club).     % работает_одинаково(бутса,дубинка)
